    ...
    inner class DeliveryViewHolder(private val binding:DeliveryItemBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(position: Int){
            binding.apply {
                ...
                val colorMap = mapOf(
                    "recaived" to Color.GREEN,"notReceived" to Color.RED,"Pending" to Color.GRAY,
                )
                statusMoney.setTextColor(colorMap[moneyStatus[position]]?:Color.BLUE)
                StatusColor.backgroundTintList = ColorStateList.valueOf(colorMap[moneyStatus[position]]?:Color.BLUE)
